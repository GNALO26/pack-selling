...
export const packsApi = {
  getAll: () => api.get<{ packs: Pack[] }>('/packs'),
  getOne: (slug: string) => api.get<{ pack: Pack }>(`/packs/${slug}`),
};
...
export const adminApi = {
  ...
  grantAccess: (userId: string, packId: string) => api.post('/admin/grant-access', { userId, packId }),
  revokeAccess: (userId: string, packId: string) => api.post('/admin/revoke-access', { userId, packId }),
...
export interface User {
  ...
  role: 'client' | 'admin'; isEmailVerified: boolean;
  purchases: Purchase[]; lastLoginAt?: string; loginCount: number;
  isActive: boolean; createdAt: string;
}
export interface Purchase {
  _id: string; packId: PackInfo | string;
  purchasedAt: string; amount: number; currency: string;
